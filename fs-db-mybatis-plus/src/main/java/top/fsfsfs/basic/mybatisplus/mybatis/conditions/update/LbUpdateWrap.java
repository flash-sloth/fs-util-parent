package top.fsfsfs.basic.mybatisplus.mybatis.conditions.update;
 ...
 *
 * @author tangyh
 * @since Created on 2019/5/27 17:15
 */
public class LbUpdateWrap<T> extends AbstractLambdaWrapper<T, LbUpdateWrap<T>>
    ...
    public LbUpdateWrap<T> like(SFunction<T, ?> column, Object val) {
        return super.like(checkCondition(val), column, SqlUtils.keywordConvert(val));
    }

    @Override
    public LbUpdateWrap<T> notLike(SFunction<T, ?> column, Object val) {
        return super.notLike(checkCondition(val), column, SqlUtils.keywordConvert(val));
    }

    @Override
    public LbUpdateWrap<T> likeLeft(SFunction<T, ?> column, Object val) {
        return super.likeLeft(checkCondition(val), column, SqlUtils.keywordConvert(val));
    }

    @Override
    public LbUpdateWrap<T> likeRight(SFunction<T, ?> column, Object val) {
        return super.likeRight(checkCondition(val), column, SqlUtils.keywordConvert(val));
    }
