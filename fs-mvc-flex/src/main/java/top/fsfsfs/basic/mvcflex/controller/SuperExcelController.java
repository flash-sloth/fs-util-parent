...
import top.fsfsfs.basic.validator.utils.ValidatorUtils;
 ...
 */
@Slf4j
public abstract class SuperExcelController<S extends SuperService<Entity>,
        Id extends Serializable, Entity extends BaseEntity<Id>, DTO, Query, VO>
    ...
    protected Class<DTO> currentDtoClass() {
        return (Class<DTO>) TypeUtil.getTypeArgument(this.getClass(), 3);
    }
    ...
    public void exportExcel(@RequestBody @Validated PageParams<Query> params, HttpServletRequest request, HttpServletResponse response) throws IOException {
        try {
     ...
     */
    public List<?> findExportList(PageParams<Query> params) {
    ...
    public R<Boolean> importExcel(@RequestParam(value = "file") MultipartFile simpleFile,
        ...
        // 配置excel第一行字段名
        try {
            List<DTO> dataList = EasyExcel.read(simpleFile.getInputStream()).head(getDtoClass()).sheet().doReadSync();
     ...
     */
    protected List<Entity> validData(List<DTO> dataList) {
        // 业务校验
